  ...
  try {
    ...
    const [programs, total] = await Promise.all([
      prisma.program.findMany({
        where,
        include: { _count: { select: { subjects: true, courses: true } } },
    ...
    return NextResponse.json({ programs, total, page, pages: Math.ceil(total / limit) });
  } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const program = await prisma.program.create({
      data: { name, slug, description: description ?? null, icon: icon ?? null, color: color ?? null, sortOrder: sortOrder ?? 0 },
    });

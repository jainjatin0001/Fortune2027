...
export async function PATCH(
  ...
  try {
    ...
    const allowed = ['title', 'content', 'isActive', 'targetRole', 'startDate', 'endDate'];
    const raw = Object.fromEntries(Object.entries(body).filter(([k]) => allowed.includes(k)));

    const data: Record<string, unknown> = { ...raw };
    if (raw.startDate) data.startDate = new Date(raw.startDate as string);
    if (raw.endDate) data.endDate = new Date(raw.endDate as string);
